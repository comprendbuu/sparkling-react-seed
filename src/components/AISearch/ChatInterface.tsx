...
export default function ChatInterface({
  ...
  return <div ref={containerRef} className="w-full h-screen bg-background overflow-hidden">
      <div className="flex flex-col h-full">
        <ScrollArea ref={scrollAreaRef} scrollHideDelay={0} className="flex-1 px-4 overflow-auto chat-scroll-area">
          <div className="space-y-4 py-4">
            {messages.map((message, index) => <div key={index}>
                {message.type === "question" ? <ChatBubbleQuestion message={message.content as string} /> : <ChatBubbleAnswerContainer claims={message.content as Answer[]} hoveredSourceId={hoveredSourceId} onSourceHover={handleSourceHover} sources={sources} query={currentQuery} threadId={threadId} mapSourceIndex={mapSourceIndex} containerRef={containerRef} />}
              </div>)}

            {isSearchLoading && <div className="flex justify-center w-full">
        ...
        <div className="bg-white">
          <SearchInput onSearch={onSearch} isLoading={isLoading} isSearchBarActive={isSearchBarActive} isFollowup={messages.length > 0} selectedSources={selectedSources} />
        </div>
      </div>
    </div>;
}